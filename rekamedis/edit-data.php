...
$sqlrm = "SELECT *, tbl_pasien.alamat AS alamatpasien FROM tbl_rekammedis INNER JOIN tbl_pasien ON tbl_rekammedis.id_pasien = tbl_pasien.id INNER JOIN tbl_user ON 
tbl_rekammedis.id_dokter = tbl_user.user_id WHERE no_rm = '$id'";
      ...
      <form action="proses-data.php" method="post">
        <div class="row">
            <div class="col-lg-6 pe-4">
                ...
                <div class="form-group mb-3">
                    <label for="tgl" class="form-label">Pasien</label>
                ...
                <div class="form-group mb-3">
                    ...
                    <textarea name="keluhan" id="keluhan" class="form-control border-0" placeholder="keluhan pasien tentang penyakitnya"><?= $rm['keluhan'] ?></textarea>
                </div>
            ...
            <div class="col-lg-6 border-start ps-4">
                <div class="form-group mb-3">
                    ...
                    <select name="dokter" id="dokter" class="form-select">
                      <?php
                      $queryDokter = mysqli_query($koneksi, "SELECT * FROM tbl_user WHERE jabatan = 2");
                      while ($data = mysqli_fetch_assoc($queryDokter)) { ?>
                          <option value="<?= $data['user_id'] ?>"
                          <?= $data['user_id'] == $rm['id_dokter'] ? 'selected' : '' ?>><?= $data['fullname'] ?></option>
                      <?php } ?>
                    </select>
                </div>
                <div class="form-group mb-3">
                    <label for="diagnosa" class="form-label">Diagnosa</label>
                    <textarea name="diagnosa" id="diagnosa" class="form-control border-0" placeholder="hasil diagnosa dokter"><?= $rm['diagnosa'] ?></textarea>
                </div>
                <div class="form-group mb-3">
